...
namespace Thirdweb.Unity
{
    public class MetaMaskWallet : IThirdwebWallet
    {
        ...
        public static async Task<MetaMaskWallet> Create(
        ...
        )
        {
            ...
            {
                throw new Exception(
                    "MetaMaskWallet is only available in WebGL Builds. Please use a different wallet provider on native platforms."
                );
            }
            ...
            if (
                metaMaskInstance.IsConnected()
                && !string.IsNullOrEmpty(metaMaskInstance.GetAddress())
            )
            {
        ...
        public Task<string> PersonalSign(byte[] rawMessage)
        {
            if (rawMessage == null)
            {
                throw new ArgumentNullException(
                    nameof(rawMessage),
                    "Message to sign cannot be null."
                );
            }
        ...
        public async Task<string> PersonalSign(string message)
        {
            if (string.IsNullOrEmpty(message))
            {
                throw new ArgumentNullException(
                    nameof(message),
                    "Message to sign cannot be null or empty."
                );
            }
            ...
            var rpcRequest = new RpcRequest
            {
                ...
                Params = new object[] { message, WebGLMetaMask.Instance.GetAddress() }
            };
        ...
        public async Task<ThirdwebTransactionReceipt> ExecuteTransaction(
            ThirdwebTransactionInput transaction
        )
        {
            var hash = await SendTransaction(transaction);
            return await ThirdwebTransaction.WaitForTransactionReceipt(
                client: _client,
                chainId: WebGLMetaMask.Instance.GetActiveChainId(),
                txHash: hash
            );
        }
        ...
        public async Task<string> SignTypedDataV4(string json)
        {
            ...
            var rpcRequest = new RpcRequest
            {
                ...
                Params = new object[] { await GetAddress(), json }
            };
            return await WebGLMetaMask.Instance.RequestAsync<string>(rpcRequest);
        }
            ...
            where TDomain : IDomain
        {
            ...
            if (typedData == null)
            {
                throw new ArgumentNullException(
                    nameof(typedData),
                    "Typed data to sign cannot be null."
                );
            }
        ...
        public Task<string> RecoverAddressFromTypedDataV4<T, TDomain>(
            T data,
        ...
        {
            var switchEthereumChainParameter = new SwitchEthereumChainParameter
            {
            ...
            var rpcRequest = new RpcRequest
            {
                ...
                Params = new object[] { switchEthereumChainParameter }
            };
        ...
        private static async Task AddNetwork(BigInteger chainId)
        {
            ...
            var twChainData =
                await Utils.GetChainMetadata(_client, chainId)
                ?? throw new Exception($"Chain data for chainId {chainId} could not be fetched.");
            ThirdwebDebug.Log($"Chain data fetched: {JsonConvert.SerializeObject(twChainData)}");
            var explorers =
                twChainData.Explorers?.Select(e => e?.Url).Where(url => url != null).ToList()
                ?? new List<string>();
            var iconUrl =
                twChainData.Icon?.Url ?? "ipfs://QmdwQDr6vmBtXmK2TmknkEuZNoaDqTasFdZdu3DRw8b2wt";
            ...
            var rpcRequest = new RpcRequest
            {
                ...
                Params = new object[] { addEthereumChainParameter }
            };
            _ = await WebGLMetaMask.Instance.RequestAsync<string>(rpcRequest);
